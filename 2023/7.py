...
p1 = sum((ind + 1) * hand[1] for ind, hand in enumerate(sorted(parsed_input, key=lambda i: val(i[0]))))
...
def val(hand):
    ...
    return types.index(typ) * 16 ** 5 + sum(16 ** (4 - i) * strength.index(hand[i]) for i in range(5))
p2 = sum((ind + 1) * hand[1] for ind, hand in enumerate(sorted(parsed_input, key=lambda i: val(i[0]))))
