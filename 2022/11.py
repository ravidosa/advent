...
parsed_input = parser(inp, "Monkey {{i}}: Starting items: {{li, }} Operation: new = old {{e}} {{e}} Test: divisible by {{i}} If true: throw to monkey {{i}} If false: throw to monkey {{i}}")

oper = lambda old, op: old ** 2 if op == ["*", "old"] else old * op[1] if op[0] == "*" else old + op[1]
def monkeying(rounds, worry):
    monkeys = {i[0]: {"it": i[1].copy(), "op": [i[2], i[3]], "div": i[4], "thr": [i[6], i[5]], "ins": 0} for i in parsed_input}
...
divvy = math.lcm(*map(lambda i: i[4], parsed_input))
