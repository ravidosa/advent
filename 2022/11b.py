...
divvy = reduce((lambda x, y: x * y), [value["test"] for value in m.values()])
for r in range(10000):
    for key, value in m.items():
        value["items"] = [((it ** 2 if value["op"] == "* old" else (it + int(value["op"][2:]) if value["op"][0] == "+" else it * int(value["op"][2:])))) % divvy for it in value["items"]]
